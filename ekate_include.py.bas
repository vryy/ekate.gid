...
*if(strcmp(GenData(Parallel_Execution),"mpi")==0)
        ...
        self.solver = trilinos_contact_structural_solver.SolverAdvanced( self.model_part, self.domain_size, number_of_time_steps, self.analysis_parameters, self.abs_tol, self.rel_tol )
...
*if(strcmp(GenData(Enable_Mortar_Contact),"1")==0)
        import mortar_contact_strategy
        self.solver = mortar_contact_strategy.SampleSolver(self.model_part, self.abs_tol, self.rel_tol)
        mortar_contact_strategy.AddVariables( self.model_part )
*else
...
*if(strcmp(GenData(Parallel_Execution),"serial")==0)
*if(strcmp(GenData(Enable_Mortar_Contact),"1")==0)
        mortar_contact_strategy.AddDofs( self.model_part )
*else
